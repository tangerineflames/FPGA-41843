...
// 删除倒计时以节省LUT资源
// 优先级：两个都拨=平衡；只�?? LR=01；只�?? TB=10；都不拨=00
assign car_bias =
    (car_sw_s1 & car_sw2_s1) ? 2'b00 :   // 两个同时�?? �?? EQ
...
always @(posedge vga_clk or negedge sys_rst_n) begin
  if (!sys_rst_n) begin
    ...
    // ================= �����ж�������Բ�?+��ȫ���� -> wrap�� =================
    // ��ࣺdir_left=+1 ��->�£�dir_left=-1 ��->��
    if (entered_left) begin
      if (dir_left ==  1) begin
        // ͷ��Խ�� YB + ���������� PED_N/PED_STEP/PED_R��
